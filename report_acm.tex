...
These different components will discussed in more detailed in the next sections.
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
				\begin{figure}[htb]
  				\begin{center}
...
Let's take the following example: a computational unit send the proximity sensor message shown earlier, the Central Unit will in return respond with the following message:
...
We choose those six contexts because they cover many facets of the Central Unit (web interface, data aggregation or even information retrial over the Internet).
...
%stats presentations
The interface must also display informations that are gartered by the system to monitor the building activity.
...
A problem occurs when dealing with more complex wired structures. We here only discuss of the "schema 3" type that allows to control one single light
...
Another additional feature would be to add a little portable computer as a tablet at the entrance of each apartment 
...
Firstly, designing a reliable system involves more components that we originally thought. 
...
Concerning the web interface, a more user-friendly feature such as a Facebook look-like time line to monitor the house activity could allow a better user experience, as most of them are now used this kind of data presentation. 

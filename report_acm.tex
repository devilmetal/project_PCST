...
In that case it is possible to let the actual wire structure as it is and only replace the switch by our smart switch.
...
A motor is composed of two input and one output. Each input control a behavior (up or down).
...
Failure handling is the most complex part of this system because we need to detect many thing as broken actuators, broken sensors, 
network failures or to deal with a central unit crash.
\subsection{Sensors or actuator failure}
It is possible to detect a sensor or actuator failure. 
...
In general sensor failures are easy to detect because the small computer that aggregate its sensor value can detect that a sensors no more send values.
Different sensors behavior as sensors degradation of skew sensors values can be detected via simple machine learning as computing mean and variance of sensor outputs and detect abnormal variations via a probabilist model.
After a sensors failure detection, the sensors send a message to the central unit.
\begin{verbatim}
POST address 
  <MESSAGE>
    <TYPE>failure</TYPE>
...
For example if the rooms sensors return a "dark" state for a room and a light is activated in the room. Then the light bulb might be broken.
In that case no message is passed as the central unit itself compute this failure.
\subsection{Smart switch failure}
The failure of a smart switch can lead to very problematic behavior such as the total incapacity of making actuators works.
This will lead to not beeing able anymore to use the lights or to open the sunblinds.
The mean idea here is to let the user have a control on the switches physically with a standard switch button.
The problem still exists if the switches are part of a "shema 3" for example. The users need to activate the all the switches to be able to light a bulb on such a configuration.
...
Allowing the user to have little physical control on the switch is a good solution to begin with.
Another additional feature would be to add a little portable computer as a tablet at the entrance of each appartment to let the user bypass the central unit messages and to control with a specificaly designed application
the whole appartment. 
It could also remotly reboot the central unit if needed and control the patches installation.
This little tablet could also display the errors of the system and allow a more easy fix of the failure.
...
An user-firendly feature would be to notify the users in case of failure. This can easily be done via the web interface itself as suggested by figure~\ref{mockup} or via other means such as SMS or email services.

...
\subsubsection{Sensors or actuator failure}
...
\begin{description}
 \item[Sensors] - In general sensor failures are easy to detect because the small computer that aggregate its sensor value can detect that a sensors no more send values.
 ...
 \item[Actuators] - Actuators are harder to detect. %because of no return value of actuators
\end{description}
\begin{verbatim}
POST address 
  <MESSAGE>
    <TYPE>failure</TYPE>
    <FROM>(sensor_id|actuator_id)</FROM>
    <CONTENT/>
  </MESSAGE>
\end{verbatim}
